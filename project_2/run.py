...
def run(args):
    ...
    for i in range(0, int(args.samplesize)):
        ...
        labels = np.left_shift(Y_train[:,0], 4) + Y_train[:,1]

        # lb.set_params("classes_", ["a", "b"])
        # bin_labels = lb.transform(Y_train)
        classifier_y1 = RandomForestClassifier(n_estimators=40).fit(X_train, labels)
        # classifier_y0 = OneVsRestClassifier(LinearSVC(random_state=0), n_jobs=-1).fit(X_train, labels)
